...
Процедура СформироватьОтчетРасходы(Команда)
	...
	ПоказатьСводныйОтчетПоПериоду("Месяц");
...
Функция СобратьHTMLСтраницыВсехСчетов()
	...
	Строки.Добавить("body{font-family:-apple-system,BlinkMacSystemFont,""Segoe UI"",Roboto,Arial,sans-serif;background:#fff;color:#1c2430;padding:12px 14px 24px;line-height:1.3}");
...
Функция ТекстМакетаГлавнойСтраницы()
	...
	Строки.Добавить(".nav{position:fixed;left:0;right:0;bottom:0;background:#fff;border-top:1px solid #e6ebf0;display:flex;padding:6px 8px calc(6px + env(safe-area-inset-bottom,0px));z-index:10}");
	Строки.Добавить(".nav a{flex:1;text-align:center;padding:4px;color:#8b95a5;font-size:10px;font-weight:600}");
	...
	Строки.Добавить("<a class=""active"" href=""app:accounts-all""><span class=""ico"">" + SVGИконкаКошелек("currentColor") + "</span>Счета</a>");
...
Процедура ОбработатьКомандуАналитики(КомандаПриложения)
	...
	Если СтрНачинаетсяС(КомандаПриложения, "period:") Тогда
		...
		ИначеЕсли РежимАналитики <> "Счета" Тогда
