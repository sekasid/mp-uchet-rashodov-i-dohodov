...
Функция СобратьHTMLГлавнойСтраницы()
	...
	КлассОстатка = ?(ДанныеСчетов.Итого < 0, "neg", "");
	ДоходыТекст = ОбщийМодульСервер_КурсыВалют.ПредставлениеСуммыВВалютеУчетаДляHTML(ДанныеСводки.МесяцДоходы, Ложь);
	// В карточке «Расходы» минус не ставим — сумма оборота, знак даёт красный цвет.
	РасходыТекст = ОбщийМодульСервер_КурсыВалют.ПредставлениеСуммыВВалютеУчетаДляHTML(ДанныеСводки.МесяцРасходы, Ложь);
	
	// Hide: на главной маскируем остаток, доходы, расходы и суммы в «Последние операции».
	// Экран «Операции» и отчёты — без маски.
	Если ОбщийМодульСервер_Безопасность.СкрыватьСуммы() Тогда
		
		Маска = ЭкранироватьHTML(ОбщийМодульСервер_Безопасность.МаскаСуммы());
		ОстатокТекст = Маска;
		ДоходыТекст = Маска;
		РасходыТекст = Маска;
		КлассОстатка = "";
	...
	Результат = СтрЗаменить(Результат, "#КлассОстатка#", КлассОстатка);
	...
	Результат = СтрЗаменить(Результат, "#ПоследниеОперации#",
		СформироватьHTMLПоследнихОпераций(5, ОбщийМодульСервер_Безопасность.СкрыватьСуммы()));
...
Функция ТекстМакетаГлавнойСтраницы()
	...
	Строки.Добавить(".balance .val.neg{color:#DC2626}");
	...
	Строки.Добавить("<div class=""balance""><a class=""eye"" href=""app:toggle-amounts"" title=""Скрыть остаток, доходы и расходы"">"
		+ ?(ОбщийМодульСервер_Безопасность.СкрыватьСуммы(), "Show", "Hide") + "</a>"
		+ "<div class=""lbl"">Общий остаток</div><div class=""val #КлассОстатка#"">#ОбщийОстаток#</div><div class=""hint"">на всех счетах</div></div>");
...
Функция СобратьHTMLСтраницыОтчетов()
	...
	Строки.Добавить("<a class=""item"" href=""app:report:debts""><span class=""name"">Отчет по долгам</span><span class=""ico"">" + SVGИконкаДолги("#14B8A6", 18) + "</span></a>");
...
Функция СобратьHTMLСтраницыНастроек(ПоказыватьБезопасность = Ложь)
	...
	Строки.Добавить(HTMLСтрокаМеню("Обмен расходами (Excel)", "open:import-expenses"));
...
// СкрыватьСуммыОпераций — только для блока на главной; экран «Операции» всегда с суммами.
//
&НаСервере
Функция СформироватьHTMLПоследнихОпераций(ЛимитСтрок = 10, СкрыватьСуммыОпераций = Ложь)
	...
	МаскаСуммы = ?(СкрыватьСуммыОпераций, ЭкранироватьHTML(ОбщийМодульСервер_Безопасность.МаскаСуммы()), "");
	Части = Новый Массив;
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		Если СкрыватьСуммыОпераций Тогда
			
			СуммаТекст = МаскаСуммы;
			
		Иначе
			
			// Расход — ASCII «-» перед суммой; доход — без префикса.
			СуммаТекст = ОбщийМодульСервер_КурсыВалют.ПредставлениеСуммыВВалютеУчетаДляHTML(СтрокаТаблицы.Сумма, Ложь);
			
			Если ВидКласс = "expense" Тогда
				
				СуммаТекст = "-" + СуммаТекст;
...
Функция ПолучитьТаблицуПоследнихОпераций(ЛимитСтрок)
	...
	|		Долг.Дата,
	|		ВЫБОР
	|			КОГДА Долг.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииДолга.ВозвратДенег)
	|				ТОГДА ""Возврат долга""
	|			КОГДА Долг.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииДолга.ВыдачаДенег)
	|				ТОГДА ""Выдача долга""
	|			КОГДА Долг.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидОперацииДолга.БеруВДолг)
	|				ТОГДА ""Возврат долга""
	|			ИНАЧЕ ""Выдача долга""
	|		КОНЕЦ + "" · "" + ЕСТЬNULL(Долг.Контакт.Наименование, ""Долг""),
	|		Долг.Сумма,
	|		ВЫБОР
	|			КОГДА Долг.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииДолга.ВозвратДенег)
	|				ТОГДА ""income""
...
Процедура ОткрытьОтчетПоКоду(КодОтчета)
	...
	ИначеЕсли КодОтчета = "debts" Тогда
		
		ОткрытьОтчетПоДолгам();
...
&НаКлиенте
Процедура ОткрытьОтчетПоДолгам()
	
	ОткрытьФорму("Отчет.ОтчетПоДолгам.Форма.ФормаОтчета", , ЭтотОбъект, , , , ,
...
Процедура ОткрытьПунктМеню(КодПункта)
	...
	ИначеЕсли КодПункта = "backup" Тогда
		
		ОткрытьФорму("ОбщаяФорма.ФормаРезервногоКопирования", , ЭтотОбъект, , , , ,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		
	ИначеЕсли КодПункта = "import-expenses" Тогда
		
		ОткрытьФорму("Обработка.ИмпортРасходовИзФайла.Форма", , ЭтотОбъект, , , , ,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ОткрытьФормуБыстрогоВвода(ИмяВидаОперации)
	...
	ИмяФормыДокумента = ?(ИмяВидаОперации = "Расход",
		"Документ.Расход.Форма.ФормаДокумента",
		"Документ.Доходы.Форма.ФормаДокумента");
	
	ОткрытьФорму(ИмяФормыДокумента, , ЭтотОбъект, , , , ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
